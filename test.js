...
test.beforeEach(async () => {
	...
	generator = helpers.createGenerator('nm-forresst:app', ['../app'], null, {skipInstall: true});
});
...
test.serial('prompts for description', async () => {
	...
	assert.fileContent('package.json', /"description": "test : foo",/);
	assert.fileContent('readme.md', /test : foo/);
});
...
test.serial('defaults to superb description', async () => {
	...
	assert.fileContent('package.json', /"description": "test : My .+ module",/);
	assert.fileContent('readme.md', /test : My .+ module/);
});
